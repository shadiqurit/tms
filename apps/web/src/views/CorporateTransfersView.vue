...
import { corporateTransferRecords, type CorporateTransferRecord } from '../data/corporateTransfers';
...
<template>
  <section class="sites-page corporate-transfers-page">
    ...
    <div class="site-table-wrap panel corporate-table-wrap"><div v-if="loading" class="table-loading">Loading corporate transfers…</div><table v-else class="site-table corporate-transfer-table"><thead><tr><th>Transfer</th><th>From project / site</th><th>To project / site</th><th>Products</th><th>Total</th><th>Status</th><th><span class="sr-only">Actions</span></th></tr></thead><tbody>
      <tr v-for="item in visible" :key="item.id"><td><div class="expense-voucher"><span>{{ date(item.transferDate) }}</span><strong>{{ item.transferNo || `TRN-${item.id}` }}</strong></div></td><td><div class="site-project-cell"><strong>{{ item.fromProjectCode || `#${item.fromProjectId}` }}</strong><small>{{ item.fromProjectName }}</small><span class="corporate-site"><MapPin :size="13" />{{ item.fromSiteName || 'All / source project' }}</span></div></td><td><div class="site-project-cell"><strong>{{ item.receiveProjectCode || `#${item.receiveProjectId}` }}</strong><small>{{ item.receiveProjectName }}</small><span class="corporate-site"><MapPin :size="13" />{{ item.receiveSiteName || 'Defined per product line' }}</span></div></td><td><span class="corporate-line-count"><PackageCheck :size="15" /><strong>{{ item.itemCount }}</strong><small>{{ Number(item.totalQuantity).toLocaleString() }} units</small></span></td><td><strong class="expense-amount">{{ money(item.totalAmount) }}</strong></td><td><span class="status-pill" :class="item.status">{{ item.status }}</span></td><td><RouterLink :to="`/purchases/transfers/${item.id}/edit`" class="card-edit" aria-label="Modify corporate transfer"><Pencil :size="15" /></RouterLink></td></tr>
