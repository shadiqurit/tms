...
const employeeId = Number(route.params.employeeId); const projectId = Number(route.params.projectId);
const ledger = ref<EngineerLedger | null>(null); const transactions = ref<EngineerLedgerTransaction[]>([]); const sites = ref<SiteOption[]>(projectSites.filter((item) => item.projectId === projectId).map((item) => ({ id: item.id, projectId: item.projectId, name: item.name })));
...
const search = ref(''); const typeFilter = ref('all'); const page = ref(1); const pageSize = 30;
const today = new Date().toISOString().slice(0, 10);
const form = reactive<DepositInput>({ employeeId, projectId, siteId: null, depositDate: today, amount: 0, referenceNo: '', notes: '' });
...
async function load() {
  ...
  try {
    if (demoMode) { const result = getDemoEngineerLedger(employeeId, projectId); ledger.value = result.ledger; transactions.value = result.transactions; }
    else {
      const [result, options] = await Promise.all([
        api<{ ledger: EngineerLedger; transactions: EngineerLedgerTransaction[] }>(`/engineer-deposits/ledger?employeeId=${employeeId}&projectId=${projectId}`),
        api<{ sites: SiteOption[] }>('/engineer-deposits/options'),
      ]);
      ledger.value = result.ledger; transactions.value = result.transactions; sites.value = options.sites.filter((item) => item.projectId === projectId);
    }
...
function openEdit(item: EngineerLedgerTransaction) { editingId.value = item.id; Object.assign(form, { employeeId, projectId, siteId: item.siteId, depositDate: item.transactionDate ?? '', amount: item.credit, referenceNo: item.referenceNo ?? '', notes: item.notes ?? '' }); formError.value = ''; showForm.value = true; }
...
function exportLedger() {
  ...
  const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `${ledger.value.employeeCode || ledger.value.employeeId}-${ledger.value.projectCode}-ledger.csv`; link.click(); URL.revokeObjectURL(link.href);
}
</script>
...
<template>
  <section class="sites-page engineer-ledger-page">
    ...
    <template v-if="ledger">
      <div class="ledger-hero panel"><div class="ledger-person"><span>{{ ledger.employeeName.split(' ').map((part) => part[0]).join('').slice(0,2) }}</span><div><small>Site engineer ledger</small><h2>{{ ledger.employeeName }}</h2><p>{{ ledger.employeeCode || `Employee #${ledger.employeeId}` }} · {{ ledger.projectCode }} — {{ ledger.projectName }}</p></div></div><div class="ledger-actions"><button class="secondary-button" @click="exportLedger"><Download :size="16" /> Export CSV</button><button v-if="canManage" class="primary-button" @click="openNew"><Plus :size="17" /> Add deposit</button></div></div>
      <div class="ledger-balance-grid">
        <article class="panel"><span><ArrowDownToLine :size="18" /></span><div><small>Deposited</small><strong>{{ money(ledger.deposited) }}</strong><em>{{ counts.deposits }} transactions</em></div></article>
        <article class="panel"><span><PackageSearch :size="18" /></span><div><small>Materials</small><strong>{{ money(transactions.filter((item) => item.type === 'material').reduce((sum, item) => sum + item.debit, 0)) }}</strong><em>{{ counts.materials }} purchase lines</em></div></article>
        ...
        <article class="panel net" :class="{ negative: ledger.balance < 0 }"><span><WalletCards :size="18" /></span><div><small>Current balance</small><strong>{{ money(ledger.balance) }}</strong><em>{{ ledger.balance < 0 ? 'Overspent — review required' : 'Available with engineer' }}</em></div></article>
      </div>
      <div class="ledger-explanation panel"><BanknoteArrowDown :size="19" /><p><strong>Balance = deposits − materials − other expenses.</strong> Every purchase line is retained below with its date, project site, purchase reference, and running balance.</p></div>
      <div class="site-toolbar panel ledger-toolbar"><label class="table-search"><Search :size="17" /><input v-model="search" placeholder="Search reference, site, details, notes, or date…" /></label><select v-model="typeFilter"><option value="all">All transactions</option><option value="deposit">Deposits only</option><option value="material">Materials only</option><option value="expense">Other expenses only</option></select></div>
      <div class="site-table-wrap panel ledger-table-wrap"><table class="site-table engineer-transaction-table"><thead><tr><th>Date / type</th><th>Reference</th><th>Site</th><th>Transaction details</th><th>Deposit</th><th>Purchase / expense</th><th>Running balance</th><th><span class="sr-only">Actions</span></th></tr></thead><tbody>
        <tr v-for="item in visible" :key="`${item.type}-${item.id}`"><td><div class="ledger-kind"><span :class="item.type">{{ item.type === 'deposit' ? 'Deposit' : item.type === 'material' ? 'Material' : 'Expense' }}</span><small>{{ date(item.transactionDate) }}</small></div></td><td><RouterLink v-if="item.purchaseId" :to="`/purchases/site-engineer/${item.purchaseId}/edit`" class="ledger-reference">{{ item.referenceNo }}</RouterLink><strong v-else class="ledger-reference plain">{{ item.referenceNo || `DEP-${item.id}` }}</strong></td><td><span class="ledger-site">{{ item.siteName || 'Project-wide' }}</span></td><td><div class="ledger-description"><strong>{{ item.description }}</strong><small v-if="item.notes && item.notes !== item.description">{{ item.notes }}</small></div></td><td><strong v-if="item.credit" class="ledger-credit">+ {{ money(item.credit) }}</strong><span v-else>—</span></td><td><strong v-if="item.debit" class="ledger-debit">− {{ money(item.debit) }}</strong><span v-else>—</span></td><td><strong class="deposit-balance" :class="{ negative: item.balance < 0 }">{{ money(item.balance) }}</strong></td><td><div v-if="item.type === 'deposit'" class="line-actions"><button v-if="canManage" class="line-edit" aria-label="Edit deposit" @click="openEdit(item)"><Pencil :size="14" /></button><button v-if="canDelete" class="line-delete" aria-label="Delete deposit" @click="removeDeposit(item)"><Trash2 :size="14" /></button></div></td></tr>
        <tr v-if="!visible.length"><td colspan="8"><div class="site-empty"><WalletCards :size="24" /><strong>No transactions found</strong><span>Change the filter or add a deposit.</span></div></td></tr>
    ...
    <div v-if="showForm" class="modal-backdrop" @click.self="showForm = false"><form class="expense-line-modal deposit-entry-modal" @submit.prevent="saveDeposit"><header><span><ArrowDownToLine :size="18" /></span><div><h3>{{ editingId ? 'Modify deposit' : 'Add deposit' }}</h3><p>{{ ledger?.employeeName }} · {{ ledger?.projectCode }}</p></div><button type="button" class="modal-close" @click="showForm = false"><X :size="18" /></button></header><div class="form-grid two">
      ...
      <label class="form-field"><span>Site (optional)</span><select v-model="form.siteId"><option :value="null">Project-wide</option><option v-for="site in sites" :key="site.id" :value="site.id">{{ site.name }}</option></select></label><label class="form-field"><span>Reference</span><input v-model="form.referenceNo" maxlength="120" placeholder="Cheque, voucher, or transfer no." /></label>
