...
import { corporateCategories as demoCategories, corporateLines, corporateProducts as demoProducts, corporatePurchaseRecords, corporateSubcategories as demoSubcategories, nextCorporateLineId, nextCorporatePurchaseId, type CorporateCategoryOption, type CorporateLineRecord, type CorporateProductOption, type CorporatePurchaseRecord, type CorporateSubcategoryOption } from '../data/corporatePurchases';
...
interface CorporateOptions { projects: ProjectOption[]; sites: SiteOption[]; suppliers: PurchaseOption[]; categories: CorporateCategoryOption[]; subcategories: CorporateSubcategoryOption[]; products: CorporateProductOption[]; units: PurchaseOption[] }
...
const subcategories = ref<CorporateSubcategoryOption[]>([...demoSubcategories]);
...
const lineForm = reactive({ id: null as number | null, categoryId: null as number | null, subcategoryId: null as number | null, productId: 0, unitId: null as number | null, siteId: null as number | null, quantity: 1 as number | '', unitPrice: '' as number | '', discount: '' as number | '', notes: '' });
...
onMounted(async () => {
  ...
  try {
    if (import.meta.env.VITE_DEMO_MODE === 'false') {
      ...
      categories.value = options.categories; subcategories.value = options.subcategories; products.value = options.products; units.value = options.units;
    }
...
function categoryChanged() {
  ...
  if (lineForm.subcategoryId && !availableSubcategories.value.some((item) => item.id === lineForm.subcategoryId)) lineForm.subcategoryId = null;
}
...
async function saveLine() {
  ...
  try {
    ...
    if (import.meta.env.VITE_DEMO_MODE === 'false') await api(lineForm.id ? `/corporate-purchases/${purchaseId.value}/lines/${lineForm.id}` : `/corporate-purchases/${purchaseId.value}/lines`, { method: lineForm.id ? 'PUT' : 'POST', body: JSON.stringify(payload) });
      ...
      const product = products.value.find((item) => item.id === payload.productId)!; const category = categories.value.find((item) => item.id === payload.categoryId); const subcategory = subcategories.value.find((item) => item.id === payload.subcategoryId); const unit = units.value.find((item) => item.id === payload.unitId); const site = sites.value.find((item) => item.id === payload.siteId);
      const record: CorporateLineRecord = { id: lineForm.id ?? nextCorporateLineId(), purchaseId: purchaseId.value, categoryId: payload.categoryId, categoryName: category?.name ?? '', subcategoryId: payload.subcategoryId, subcategoryName: subcategory?.name ?? '', productId: product.id, productName: product.name, unitId: payload.unitId, unitName: unit?.name ?? '', siteId: payload.siteId, siteName: site?.name ?? '', quantity: payload.quantity, unitPrice: payload.unitPrice, discount: payload.discount, totalAmount: lineTotal.value, notes: payload.notes };
...
<template>
  <section class="project-form-page corporate-purchase-form-page">
    ...
    <section v-if="editing" class="purchase-entries panel"><header class="purchase-entry-summary"><div><button class="active"><PackagePlus :size="17" /><span><strong>Corporate products</strong><small>{{ summary.itemCount }} lines</small></span></button></div><strong>{{ money(summary.totalAmount) }} total</strong></header>
      ...
      <form v-if="lineFormOpen" class="purchase-line-form corporate-line-form" @submit.prevent="saveLine"><header><div><PackagePlus :size="18" /><span><strong>{{ lineForm.id ? 'Modify product line' : 'Add corporate product' }}</strong><small>Total is calculated from quantity × price − discount.</small></span></div><button type="button" @click="lineFormOpen = false"><X :size="17" /></button></header><div class="form-grid three">
        <label class="form-field"><span>Category</span><select v-model="lineForm.categoryId" @change="categoryChanged"><option :value="null">All categories</option><option v-for="item in categories" :key="item.id" :value="item.id">{{ item.name }}</option></select></label>
        <label class="form-field"><span>Subcategory</span><select v-model="lineForm.subcategoryId"><option :value="null">No subcategory</option><option v-for="item in availableSubcategories" :key="item.id" :value="item.id">{{ item.name }}</option></select></label>
        <label class="form-field"><span>Find product</span><div><Search :size="15" /><input v-model="productSearch" placeholder="Type product name…" /></div></label>
        <label class="form-field full"><span>Corporate product <b>*</b></span><select v-model.number="lineForm.productId" @change="productChanged"><option :value="0" disabled>Select product ({{ availableProducts.length }} available)</option><option v-for="item in availableProducts" :key="item.id" :value="item.id">{{ item.name }}{{ item.unitName ? ` — ${item.unitName}` : '' }}</option></select></label>
        <label class="form-field"><span>Delivery site</span><select v-model="lineForm.siteId"><option :value="null">Use purchase / general site</option><option v-for="site in availableSites" :key="site.id" :value="site.id">{{ site.name }}</option></select></label>
        <label class="form-field"><span>Unit</span><select v-model="lineForm.unitId"><option :value="null">No unit</option><option v-for="unit in units" :key="unit.id" :value="unit.id">{{ unit.name }}</option></select></label>
        <label class="form-field"><span>Quantity <b>*</b></span><div><input v-model.number="lineForm.quantity" type="number" min="0.0001" step="0.0001" /></div></label>
        <label class="form-field"><span>Unit price <b>*</b></span><div><input v-model.number="lineForm.unitPrice" type="number" min="0" step="0.0001" /></div></label>
        <label class="form-field"><span>Discount</span><div><input v-model.number="lineForm.discount" type="number" min="0" step="0.01" /></div></label>
        <label class="form-field full"><span>Notes</span><div><input v-model="lineForm.notes" placeholder="Optional product notes" /></div></label>
      </div><footer><span>Calculated total <strong>{{ money(lineTotal) }}</strong></span><button type="button" class="secondary-button" @click="lineFormOpen = false">Cancel</button><button class="primary-button" :disabled="lineSaving"><Check :size="15" />{{ lineSaving ? 'Saving…' : 'Save product' }}</button></footer></form>

      <div class="purchase-lines-table-wrap"><table class="site-table purchase-lines-table corporate-lines-table"><thead><tr><th>Product</th><th>Category</th><th>Site</th><th>Quantity</th><th>Unit price</th><th>Discount</th><th>Total</th><th><span class="sr-only">Actions</span></th></tr></thead><tbody><tr v-for="item in lines" :key="item.id"><td><strong>{{ item.productName }}</strong><small>{{ item.subcategoryName || item.notes }}</small></td><td>{{ item.categoryName || '—' }}</td><td>{{ item.siteName || 'General project' }}</td><td>{{ Number(item.quantity).toLocaleString() }} {{ item.unitName }}</td><td>{{ money(item.unitPrice) }}</td><td>{{ money(item.discount) }}</td><td><strong>{{ money(item.totalAmount) }}</strong></td><td><div class="line-actions"><button type="button" @click="editLine(item)"><Pencil :size="14" /></button><button type="button" class="danger" @click="pendingLineDelete = item.id"><Trash2 :size="14" /></button></div></td></tr><tr v-if="!lines.length"><td colspan="8"><div class="site-empty"><PackagePlus :size="22" /><strong>No corporate product lines found</strong><span>Add the first product to this purchase.</span></div></td></tr></tbody></table></div>
