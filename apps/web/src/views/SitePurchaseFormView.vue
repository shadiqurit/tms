...
function editMaterial(item: MaterialLineRecord) { Object.assign(materialForm, { id: item.id, materialId: item.materialId, unitId: item.unitId, siteId: item.siteId, entryDate: dateValue(item.entryDate), quantity: Number(item.quantity), unitPrice: Number(item.unitPrice), discount: Number(item.discount || 0), notes: item.notes ?? '' }); lineFormOpen.value = true; }
function editExpense(item: SiteExpenseLineRecord) { Object.assign(expenseForm, { id: item.id, expenseHeadId: item.expenseHeadId, siteId: item.siteId, entryDate: dateValue(item.entryDate), amount: Number(item.amount), notes: item.notes ?? '' }); lineFormOpen.value = true; }
