    ...
    <div class="expense-setup-layout">
      <form class="panel setup-entry-panel" @submit.prevent="saveSetup">
        ...
        <div v-if="tab === 'general'" class="setup-form-fields">
          ...
          <label class="form-field"><span>Cost stage</span><select v-model="typeForm.phase"><option value="pre_award">Pre-award cost</option><option value="execution">Project cost</option></select></label>
          <label class="check-field"><input v-model="typeForm.refundable" type="checkbox" /><span><strong>Refundable</strong><small>Track returned security or guarantee amounts.</small></span></label>
      ...
      <section class="panel setup-list-panel">
        ...
        <div v-else class="setup-record-list">
          <article v-for="item in (tab === 'general' ? filteredTypes : filteredHeads)" :key="item.id"><div><strong>{{ item.name }}</strong><span v-if="tab === 'general'"><em>{{ (item as ExpenseTypeRecord).phase === 'pre_award' ? 'Pre-award' : 'Project cost' }}</em><small>{{ (item as ExpenseTypeRecord).refundable ? 'Refundable' : 'Non-refundable' }}</small></span><span v-else><em>{{ (item as ExpenseHead).status }}</em><small>{{ item.usageCount }} entries</small></span></div><button class="card-edit" @click="tab === 'general' ? editType(item as ExpenseTypeRecord) : editHead(item as ExpenseHead)"><Pencil :size="14" /></button><button class="card-edit danger" @click="requestDelete(tab, item)"><Trash2 :size="14" /></button></article>
