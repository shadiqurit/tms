...
export const demoEngineerDeposits = legacyDepositsJson.recordset.map((item) => ({
    ...item, engineerId: legacyAccounts.find((account) => account.ID === item.PID)?.ENG_ID ?? 0, referenceNo: '', notes: '',
}));
const round = (value) => Math.round(value * 100) / 100;
function identity(employeeId) {
...
export function getDemoEngineerLedger(employeeId) {
    const purchases = purchaseHeaders.filter((item) => item.SE_ID === employeeId);
    const purchaseIds = purchases.map((item) => item.ID);
    const transactions = [];
    ...
    for (const item of transactions) {
        runningCents += Math.round(item.credit * 100) - Math.round(item.debit * 100);
    ...
    return {
        ledger: { ...identity(employeeId), walletId: employeeId, deposited, purchased, balance: runningCents / 100 },
