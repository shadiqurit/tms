...
export const useAuthStore = defineStore('auth', {
  state: () => ({
    user: JSON.parse(localStorage.getItem('tms_user') ?? 'null') as User | null,
    permissions: JSON.parse(localStorage.getItem('tms_permissions') ?? '[]') as string[],
    loading: false,
  ...
  getters: {
    ...
    hasPermission: (state) => (required?: string | string[]) => {
      if (!required || state.user?.role.key === 'programmer') return true;
  ...
  actions: {
    async login(login: string, password: string, remember = false) {
      ...
      try {
        if (demoMode) {
          ...
          const result = await api<{ token: string; user: User; permissions: string[] }>('/auth/login', {
            method: 'POST',
    ...
    async refreshSession() {
      if (demoMode) { this.user = demoUser; this.permissions = ['*']; this.sessionChecked = true; return; }
      const result = await api<{ user: User; permissions: string[] }>('/auth/me');
      this.user = result.user;
      this.permissions = result.permissions;
      this.sessionChecked = true;
      localStorage.setItem('tms_user', JSON.stringify(this.user));
